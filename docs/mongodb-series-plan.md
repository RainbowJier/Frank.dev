...
> 规划创建于 2026-08-27，**2026-08-27 已完成 01–06 六篇**（阶段一、二、三收官；各配 3 张 SVG 共 18 张，构建与产物级校验通过）。风格沿用《Elasticsearch 从零到一》（10 篇）与《RuoYi 从零到一》（12 篇），共 **12 篇**。
>
...
| 04 | 数据建模 | ✅ 已写完（2026-08-27，`04-mongodb-data-modeling.md` + 3 SVG） |
| 05 | 聚合管道（上） | ✅ 已写完（2026-08-27，`05-mongodb-aggregation-pipeline-basics.md` + 3 SVG） |
| 06 | 聚合管道（下） | ✅ 已写完（2026-08-27，`06-mongodb-aggregation-lookup-facet.md` + 3 SVG） |
| 07 | 索引与 explain 调优 | 未开始（下一批首篇；写完时把 06 结尾预告升级为 post_link） |
| 08 | 事务与一致性 | 未开始 |
| 09 | 副本集 | 未开始 |
| 10 | 分片集群 | 未开始 |
| 11 | 安全备份监控 | 未开始 |
